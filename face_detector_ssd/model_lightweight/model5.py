...
def base_layers(image, is_train=True):
  with tf.variable_scope(NAME):
    conv = tf.layers.conv2d(image, 32, [3, 3], [1, 1],
                            padding='SAME',
                            activation=tf.nn.relu,
                            use_bias=True,
                            trainable=is_train)
    conv = tf.layers.conv2d(conv, 32, [3, 3], [1, 1],
                            padding='SAME',
                            activation=tf.nn.relu,
                            use_bias=True,
                            trainable=is_train)
    ...
    conv = tf.layers.conv2d(pool, 64, [3, 3], [1, 1],
                            padding='SAME',
                            activation=tf.nn.relu,
                            use_bias=True,
                            trainable=is_train)
...
def ssd_layers(base_logits, is_train=True):
  ...
  for index in range(3):
    with tf.variable_scope('ssd_block_%d' % index):
      ...
      conv = tf.layers.conv2d(conv, 128, [3, 3], [2, 2],
                              padding='SAME',
                              activation=tf.nn.relu,
                              use_bias=True,
                              trainable=is_train,
